...
void formTestTextNotoSansHintsMono_reset(AppState* state)
{
	//FormData* data = (FormData*)state->form_data;
}

void formTestTextNotoSansHintsMono_onDraw(AppState* state)
{
...
#define setTextColor	setTextRGBColor
	...
	setGFXFont(&NotoSans_Regular6pt7b);
	setTextColor(text_color_1);
	drawString(10, y, "NotoSans 6, 9, mono");
	y += NotoSans_Regular6pt7b.yAdvance - 2;
	...
	setGFXFont(&NotoSans_Bold6pt7b);
	setTextColor(text_color_5);
	drawString(2, y, "Bold Text");
	y += NotoSans_Bold6pt7b.yAdvance - 2;

	setGFXFont(&NotoSans_Italic6pt7b);
	setTextColor(text_color_6);
	drawString(2, y, "Italic Text");
	y += NotoSans_Italic6pt7b.yAdvance - 2;

	setGFXFont(&NotoSans_BoldItalic6pt7b);
	setTextColor(text_color_7);
	drawString(2, y, "Bold Italic Text");
	y += NotoSans_BoldItalic6pt7b.yAdvance - 2;

	// 9pt
	setGFXFont(&NotoSans_Regular9pt7b);
	setTextColor(text_color_1);
	drawString(2, y, "0123ABCabc");
	y += NotoSans_Regular9pt7b.yAdvance - 4;

	setGFXFont(&NotoSans_Bold9pt7b);
	setTextColor(text_color_2);
	drawString(2, y, "0123ABCabc");
	y += NotoSans_Bold9pt7b.yAdvance - 4;

	setGFXFont(&NotoSans_Italic9pt7b);
	setTextColor(text_color_3);
	drawString(2, y, "0123ABCabc");
	y += NotoSans_Italic9pt7b.yAdvance - 4;

	setGFXFont(&NotoSans_BoldItalic9pt7b);
	setTextColor(text_color_4);
	drawString(2, y, "0123ABCabc");
	//y += NotoSans_BoldItalic9pt7b.yAdvance - 4;
}

void formTestTextNotoSansHintsMono_onButton(AppState* state, uint8_t btn)
{
	//FormData* data = (FormData*)state->form_data;
	switch (btn)
	{
	case BUTTON_CODE_2:
		// Change form
		state->form_code = FORM_TEXT_TEST_NOTOSANS_AUTO;
