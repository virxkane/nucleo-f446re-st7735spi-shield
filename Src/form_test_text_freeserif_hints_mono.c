...
void formTestTextFreeSerifHintsMono_reset(AppState* state)
{
	//FormData* data = (FormData*)state->form_data;
}

void formTestTextFreeSerifHintsMono_onDraw(AppState* state)
{
...
#define setTextColor	setTextRGBColor
	...
	setGFXFont(&FreeSerif6pt7b);
	setTextColor(text_color_1);
	drawString(10, y, "FreeSerif 6, 9, mono");
	y += FreeSerif6pt7b.yAdvance - 2;
	...
	setGFXFont(&FreeSerifBold6pt7b);
	setTextColor(text_color_5);
	drawString(2, y, "Bold Text");
	y += FreeSerifBold6pt7b.yAdvance - 2;

	setGFXFont(&FreeSerifItalic6pt7b);
	setTextColor(text_color_6);
	drawString(2, y, "Italic Text");
	y += FreeSerifItalic6pt7b.yAdvance - 2;

	setGFXFont(&FreeSerifBoldItalic6pt7b);
	setTextColor(text_color_7);
	drawString(2, y, "Bold Italic Text");
	y += FreeSerifBoldItalic6pt7b.yAdvance - 2;

	// 9pt
	setGFXFont(&FreeSerif9pt7b);
	setTextColor(text_color_1);
	drawString(2, y, "0123ABCabc");
	y += FreeSerif9pt7b.yAdvance - 4;

	setGFXFont(&FreeSerifBold9pt7b);
	setTextColor(text_color_2);
	drawString(2, y, "0123ABCabc");
	y += FreeSerifBold9pt7b.yAdvance - 4;

	setGFXFont(&FreeSerifItalic9pt7b);
	setTextColor(text_color_3);
	drawString(2, y, "0123ABCabc");
	y += FreeSerifItalic9pt7b.yAdvance - 4;

	setGFXFont(&FreeSerifBoldItalic9pt7b);
	setTextColor(text_color_4);
	drawString(2, y, "0123ABCabc");
	//y += FreeSerifBoldItalic9pt7b.yAdvance - 4;
}

void formTestTextFreeSerifHintsMono_onButton(AppState* state, uint8_t btn)
{
	//FormData* data = (FormData*)state->form_data;
	switch (btn)
	{
	case BUTTON_CODE_2:
		// Change form
		state->form_code = FORM_TEXT_TEST_FREESERIF_AUTO;
